...
class TreasureIslandGame():
    def __init__(self, map_file) -> None:
        self.__size, self.__turn_reveal, self.__turn_free, self.__num_region,self.__treasure_pos, self.__game_map = self.__load_map(map_file)
        agent_pos_init = (random.randint(1, self.__size-2), random.randint(1, self.__size-2))
        while astar(self.__game_map, agent_pos_init, self.__treasure_pos) == False:
            agent_pos_init = (random.randint(1, self.__size-2), random.randint(1, self.__size-2))
        self.__agent = Agent(agent_pos_init, self.__game_map, self.__turn_reveal , self.__turn_free)
        self.__pirate = Pirate(self.__game_map, self.__turn_reveal, self.__turn_free, self.__agent.get_pos())
        self.__turn = 1
    ...
    def play(self):
        ...
        while self.__agent.is_win() == False and self.__pirate.is_win() == False:
            ...
            logs["machine_turn"].append(agent_actions["machine_turn"])
            logs["human_turn"].append(agent_actions["human_turn"])
            self.__turn += 1
